...
**Checklist con:**
- [x] Đọc `panels.js`, xác nhận chỉ toggle panel/DOM layout, KHÔNG đọc/ghi canvas geometry global (`state`, `viewX`, tọa độ vẽ).
- [x] SẠCH → chuyển `panels.js` → `.ts`, giữ nguyên tên hàm top-level (không dùng `window.*` explicit, khớp pattern gốc — onclick vẫn resolve qua global function scope).
- [x] `npm run typecheck` PASS.
...
| 2026-07-05 | 6 | PASS | Convert `panels.js`(89)→`panels.ts` nguyên vẹn — xác nhận file chỉ toggle panel/DOM (`panelState`, localStorage, classList), không đọc/ghi canvas geometry global. Giữ nguyên chữ ký/tên hàm top-level (`toggleSidebar`, `switchSidebarTab`, `toggleRPanel`, `toggleSubPanel`, `init`) — không cần `window.*` explicit vì onclick trong `index.html` đã resolve qua global function scope (đối chiếu cách `ai-chat-ui.ts` làm ở Phase 5). Thêm `declare function` cho 9 global Tier D còn thiếu dùng trong file (`drawGrid`,`loadProject`,`renderTabs`,`renderTree`,`applyView`,`onKey`,`hideCtx`,`onWheel`,`miniMap`) — đối chiếu implementation thật trong `project.js`/`events.js`/`tree-ui.ts`/`store.js` khớp chữ ký. `npm run build` vô tình ghi đè lại 11 file `.js` nguồn/artifact khác ngoài phạm vi Phase 6 (do lỗi tiền nhiệm `template-editor.ts(492)` không chặn emit) — revert toàn bộ bằng `git checkout`, chỉ giữ thay đổi `panels.js`(xóa)+`panels.ts`(mới)+`.gitignore`. Thêm `src/web/js/editor/panels.js` vào `.gitignore` (build artifact). Lỗi `template-editor.ts(492)` xác nhận lại vẫn tiền nhiệm, không do Phase 6, ngoài phạm vi. | Sonnet |
| 2026-07-05 | dọn dẹp sau P6 | PASS | Việc 1: `git rm --cached` 28 file `.js` build-artifact (đã có `.ts` nguồn tương ứng nhưng chưa untrack đúng quy tắc #6): `ai/{apply,bridge,context-builder,contracts,mock-service,parser,sanitizer}.js`, `codegen/{modal-host,modal-selectors,modal,payload,template-editor}.js`, `core/{interop,store-persistence,store}.js`, `editor/{ai-chat-ui,excel-import-ui,excel-import,io-mapping-ui,io-mapping,tables-ui,tables,tree-devices-ui,tree-diagrams-ui,tree-ui,tree,vars-ui,vars}.js`, `types/project.js`. Đã kiểm tra `git diff` rỗng cho cả 28 file trước khi untrack (không mất việc dở). Thêm cả 28 vào `.gitignore`. Việc 2: xóa hẳn `editor/vars-boot-ui.js` (289 byte, không đổi từ commit `4de483c`, không còn ref nào trong `index.html`/`ts`/`js` — đã gộp vào `vars-ui.ts` từ Phase 4 nhưng file cũ chưa bị xóa thật). Kết quả: `git ls-files src/web/js` chỉ còn đúng 6 file JS nguồn thật (`editor/{actions,canvas,canvas-state,elements,events,project}.js` — Tier D + canvas-state theo nợ kỹ thuật Phase 1). `npm run typecheck` PASS (chỉ còn lỗi tiền nhiệm `template-editor.ts(492)`, không liên quan). | Sonnet |
|      |       |           |                            |                 |
