...
type IuiPhysicalIO = GrafcetStudioProject.PhysicalIO;
type IuiIOMappingEntry = GrafcetStudioProject.IOMappingEntry;

declare function renderGlobalVarTable(): void;

interface IuiAddressTarget {
  get(): string;
  set(value: string): void;
}

function ioNormalizeTag(v: unknown): string {
  return ioGetApi().ioNormalizeTag(v);
}

function ioCollectCandidateVariables(): GrafcetStudioIOMapping.CandidateVariable[] {
  return ioGetApi().ioCollectCandidateVariables(ioGetContext());
}

function ioAutoMatchEntries(): void {
  ioGetApi().ioAutoMatchEntries(ioGetContext());
  saveProject();
}
function ioBuildCandidateOptions(direction: string): string[] {
  return ioGetApi().ioBuildCandidateOptions(ioGetContext(), direction);
}

function ioResolveVariableAddressTarget(appVariable: string): IuiAddressTarget | null {
...
function ioSetEntryMapped(physicalIOId: string, appVariable: string): void {
  const entry = ioGetApi().ioSetEntryMapped(ioGetContext(), physicalIOId, appVariable);
  if (!entry) return;
  saveProject();
  renderIOMappingTable((document.getElementById('iomap-filter') as HTMLSelectElement | null)?.value || 'All');
}
function ioUnmapEntry(physicalIOId: string): void {
  const entry = ioGetApi().ioUnmapEntry(ioGetContext(), physicalIOId);
  if (!entry) return;
  saveProject();
  renderIOMappingTable((document.getElementById('iomap-filter') as HTMLSelectElement | null)?.value || 'All');
}
function ioConfirmManual(physicalIOId: string): void {
  const sel = document.getElementById('iomap-sel-' + physicalIOId) as HTMLSelectElement | null;
  if (!sel) return;
  ioSetEntryMapped(physicalIOId, sel.value || '');
}

function renderIOMappingTable(filter?: string): void {
  ...
  const byId: Record<string, IuiIOMappingEntry> = Object.create(null);
  ...
  rows.forEach(function (p) {
    const e: IuiIOMappingEntry = byId[p.id] || { physicalIOId: p.id, appVariable: '', status: 'unmatched', matchScore: 0 };
...
function clearIOMappingImport(): void {
  if (typeof ensureProjectIOMapping === 'function') ensureProjectIOMapping();
...
function exportIOCode(): void {
  ...
  (io.physicalIOs || []).forEach(function (p) {
    const e = byId[p.id] || { appVariable: '' } as IuiIOMappingEntry;
