...
type TreeUiDiagramMeta = GrafcetStudioProject.DiagramMeta;
type TreeUiDeviceType = GrafcetStudioProject.DeviceType;

declare const IO_MAPPING_TAB_ID: string;
declare const STRUCT_TAB_PREFIX: string;
declare function openStructTab(devId: string): void;
declare function openIOMappingTab(): void;
declare function openVarsTab(): void;
declare function renameCurrentDiagram(id: string): void;
declare function closeTab(id: string, e: Event): void;
declare function updateAlignBtns(): void;
declare function makeDevicesSection(): HTMLElement;
declare function makeDiagItem(d: TreeUiDiagramMeta): HTMLElement;

function treeGetApi(): GrafcetStudioTree.TreeApi {
  const api = window.GrafcetStudio && window.GrafcetStudio.tree as GrafcetStudioTree.TreeApi | undefined;
  if (!api) throw new Error('GrafcetStudio tree bridge is not loaded');
  return api;
}

function treeGetContext(): GrafcetStudioTree.TreeContext {
  return {
    project,
    now: () => Date.now(),
...
function renderTabs(): void {
  ...
  openTabs.forEach(t => {
    if(String(t.id).startsWith(STRUCT_TAB_PREFIX)) {
      ...
      tab.addEventListener('click', e=>{ if(!(e.target as HTMLElement).classList.contains('tab-close')) openStructTab(devId); });
      bar.appendChild(tab);
      return;
    }
    if(t.id === IO_MAPPING_TAB_ID) {
      ...
      tab.addEventListener('click', e=>{ if(!(e.target as HTMLElement).classList.contains('tab-close')) openIOMappingTab(); });
      bar.appendChild(tab);
      return;
    }
    ...
    tab.addEventListener('click', e=>{ if(!(e.target as HTMLElement).classList.contains('tab-close')) openTab(t.id); });
    tab.addEventListener('dblclick', e=>{ if(!(e.target as HTMLElement).classList.contains('tab-close')) renameCurrentDiagram(t.id); });
    bar.appendChild(tab);
  });
...
function treeIcon(name: string, cls=''): string {
...
function treeSectionOpen(key: string): boolean {
  return localStorage.getItem('gf2-tree-section-open-' + key) !== '0';
}

function bindTreeSectionToggle(section: HTMLElement, key: string): void {
  const head = section.querySelector<HTMLElement>('.tree-section-head');
  const body = section.querySelector<HTMLElement>('.tree-section-body');
  const toggle = section.querySelector<HTMLElement>('.tree-unit-toggle');
...
function makeUnitItem(u: GrafcetStudioProject.Unit): HTMLElement {
...
function makeDriversSection(): HTMLElement {
  const wrap = document.createElement('div');
  ...
  head.addEventListener('click', ()=>{
    const c=wrap.querySelector<HTMLElement>('.tree-drivers-body');
