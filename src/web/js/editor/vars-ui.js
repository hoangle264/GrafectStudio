...
function varsGetApi() {
  const api = window.GrafcetStudio && window.GrafcetStudio.vars;
  if (!api) throw new Error('GrafcetStudio vars bridge is not loaded');
  return api;
}

function ioGetApi() {
  const api = window.GrafcetStudio && window.GrafcetStudio.ioMapping;
  if (!api) throw new Error('GrafcetStudio IO mapping bridge is not loaded');
  return api;
}

function varsGetContext() {
  return {
    project,
...
function renderGlobalVarTable() {
  ...
  filtered.forEach(function(entry){
    ...
    if(sigList.length) {
      ...
      // Dng createElement  trnh re-render mt focus khi click
      tdTog.style.cssText = '';
      tdTog.onclick = null;
...
function gvtDeleteVar(source, key) {
  ...
  if(source === 'excel') {
    ...
    if(!confirm('Xa unit "'+(project.unitConfig[key].label||key)+'" khi Global Vars?')) return;
