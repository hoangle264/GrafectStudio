"use strict";
var GrafcetStudioAIP10Validation;
(function (GrafcetStudioAIP10Validation) {
    ...
    function makeOversizedRawProject() {
        const variables = [];
    ...
    function makeApplyContext(project, counts) {
        return {
    ...
    function cloneProposal(proposal, proposalId, label, address) {
    ...
    function runMockEndToEnd(errors) {
        ...
        if (!requestResult.ok || !requestResult.request)
            return failedApplyResult('ai-prop-p10-mock-e2e', true, 'request build failed');
        ...
        if (!mockResponse.ok || !mockResponse.proposal)
            return failedApplyResult('ai-prop-p10-mock-e2e', true, 'mock response failed');
    ...
    function runFullFlow(errors) {
        ...
        if (!requestResult.ok || !requestResult.request)
            return failedApplyResult('ai-prop-p10-full-flow', true, 'request build failed');
        ...
        if (!hostReceiveResult.ok || !hostReceiveResult.proposal)
            return failedApplyResult('ai-prop-p10-full-flow', true, 'host mock parse failed');
    ...
    function failedApplyResult(proposalId, dryRun, error) {
        return { ok: false, proposalId, dryRun, affectedIds: [], warnings: [], errors: [error], changed: false };
    }
    function runP10IntegrationSecurityValidation() {
        const errors = [];
        ...
        if (!sanitizer.ok)
            errors.push.apply(errors, sanitizer.errors);
        if (!contextBuilder.ok)
            errors.push.apply(errors, contextBuilder.errors);
        if (!parser.ok)
            errors.push.apply(errors, parser.errors);
        if (!mockService.ok)
            errors.push.apply(errors, mockService.errors);
        if (!apply.ok)
            errors.push.apply(errors, apply.errors);
    ...
    GrafcetStudioAIP10Validation.runP10IntegrationSecurityValidation = runP10IntegrationSecurityValidation;
})(GrafcetStudioAIP10Validation || (GrafcetStudioAIP10Validation = {}));
