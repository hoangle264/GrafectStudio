...
public static class AiPromptBuilder
{
    public static string BuildSystemPrompt(string intent)
    {
        var intentRequirements = intent switch
        {
            ...
            "create-flow" => "Return data.flow only. Keep the flow minimal and structurally complete. steps, transitions, and connections must use proposal node ids only, and every connection.from/to must reference an id emitted in the same proposal. Step actions must use the UI schema only: { qualifier: \"N\", variable: \"Struct.Signal\", address: \"\", time: \"\" }. Do not use variableId or expression for actions. Step number/initial/x/y are optional; the app will backfill them when missing.",
