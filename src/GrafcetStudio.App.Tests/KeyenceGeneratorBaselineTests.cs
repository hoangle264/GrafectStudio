...
public class KeyenceGeneratorBaselineTests
{
    ...
    public void GenerateUnitContent_MatchesGoldenSnapshot()
    {
        ...
        templates.LoadTemplate("uc.unitAuto", "; AUTO FLOWS\n{{#each autoFlows}}\n; Flow: {{name}} (Min: {{stepMinAddress}}, Max: {{stepMaxAddress}})\n{{#each steps}}\n; Step {{number}} ({{label}})\n{{#if isFirst}} -> SET {{execAddress}}{{else}}{{#if previousStep.doneAddress}}{{previousStep.doneAddress}}{{#if inTransitionCondition}} & {{inTransitionCondition}}{{/if}} -> SET {{execAddress}}{{/if}}{{/if}}{{#each actions}}\n{{../execAddress}}{{#if interlockAddress}} & {{interlockAddress}}{{/if}} -> OUT {{target}}{{/each}}\n{{#unless actions.length}}\n{{/unless}}{{execAddress}}{{#if outTransitionCondition}} & {{outTransitionCondition}}{{else}}{{#each actions}}{{#if complete.address}} & {{complete.address}}{{/if}}{{/each}}{{/if}} -> SET {{doneAddress}}\n{{/each}}\n{{/each}}");
        templates.LoadTemplate("uc.unitOrigin", "; ORIGIN FLOWS\n{{#each originFlows}}\n; Flow: {{name}}\n{{#each steps}}\n; Step {{number}} ({{label}})\n{{#if isFirst}} -> SET {{execAddress}}{{else}}{{#if previousStep.doneAddress}}{{previousStep.doneAddress}}{{#if inTransitionCondition}} & {{inTransitionCondition}}{{/if}} -> SET {{execAddress}}{{/if}}{{/if}}{{#each actions}}\n{{../execAddress}}{{#if interlockAddress}} & {{interlockAddress}}{{/if}} -> OUT {{target}}{{/each}}\n{{#unless actions.length}}\n{{/unless}}{{execAddress}}{{#if outTransitionCondition}} & {{outTransitionCondition}}{{else}}{{#each actions}}{{#if complete.address}} & {{complete.address}}{{/if}}{{/each}}{{/if}} -> SET {{doneAddress}}\n{{/each}}\n{{/each}}");
        templates.LoadTemplate("uc.outputs", "; DEVICE OUTPUTS\n{{#each deviceOutputGroups}}\n; Device: {{deviceLabel}} (Kind: {{deviceKind}}, Address: {{address}})\n{{#each commands}}\n; Command: {{commandId}} (Action: {{actionLabel}}, Target: {{physicalOutputRef}})\n{{#each flowCommands}}\n{{modeFlagAddress}} & {{sourceExecuteBit}}{{#if sourceDoneBit}} & !{{sourceDoneBit}}{{/if}} -> OUT {{../physicalOutputRef}}\n{{/each}}\n{{/each}}\n{{/each}}");
